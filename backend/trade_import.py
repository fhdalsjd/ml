        ...
        for trade_data in trades_data:
            try:
                ...
                if existing_trade:
                    ...
                    existing_trade.symbol = trade_data.get('symbol', existing_trade.symbol)
                    existing_trade.type = trade_type
                    existing_trade.profit = float(trade_data.get('profit', existing_trade.profit))
                    existing_trade.volume = float(trade_data.get('volume', existing_trade.volume))
                    existing_trade.open_price = float(trade_data.get('open_price', existing_trade.open_price or 0))
                    existing_trade.close_price = float(trade_data.get('close_price', existing_trade.close_price or 0))
                    existing_trade.stop_loss = float(trade_data.get('stop_loss', 0)) if trade_data.get('stop_loss') else existing_trade.stop_loss
                    existing_trade.take_profit = float(trade_data.get('take_profit', 0)) if trade_data.get('take_profit') else existing_trade.take_profit
                    existing_trade.commission = float(trade_data.get('commission', existing_trade.commission or 0))
                    existing_trade.swap = float(trade_data.get('swap', existing_trade.swap or 0))
                    ...
                    new_trade = models.Trade(
                        ...
                        open_price=float(trade_data.get('open_price', 0.0)),
                        close_price=float(trade_data.get('close_price', 0.0)),
